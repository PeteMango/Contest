...
class PlayFair:
    ...
    Constants:
        ...
        indicies (defaultdict[str, tuple[int, int]]): location of char in matrix
        matrix (List[List[str]]): 2D array of the encryption matrix
    """

    def __init__(self, encryption_key: str):
        """
        Init function for the PlayFair class
        """
        # constants
        ...
        # store the indicies of the character so that for n -> infinity
        # it is much more optimized than looping through the matrix to find the index
        ...
        # builds the matrix
        self.__build_matrix(key=encryption_key)
    ...
    def __diagraminate(self, text: str) -> List[str]:
        ...
        while index <= length - 2:
            ...
            if (digram[0] == digram[1] or
                (digram[0] == 'I' and digram[1] == 'J') or (digram[0] == 'J' and digram[1] == 'I')):
    ...
    def encrypt(self, text: str) -> str:
        ...
        for digram in digrams:
            ...
            if first_x == second_x:
                ...
                encrypted += (self.matrix[first_x][(first_y + 1) % self.cols] +
                              self.matrix[second_x][(second_y + 1) % self.cols])

            # case 2: both chars are in the same column
            elif first_y == second_y:
                # shift the chars down
                encrypted += (self.matrix[(first_x + 1) % self.rows][first_y] +
                              self.matrix[(second_x + 1) % self.rows][second_y])
    ...
    def decrypt(self, encrypted: str) -> str:
        ...
        # split into chunks of two (as it is guaranteed to be even in length)
        ...
        for digram in digrams:
            ...
            if first_x == second_x:
                ...
                decrypted += (self.matrix[first_x][(first_y + 4) % self.cols] +
                              self.matrix[second_x][(second_y + 4) % self.cols])

            # case 2: both chars are in the same column
            elif first_y == second_y:
                # shift the chars up
                # as we need to handle negative indicies, instead of -1 do +4
                decrypted += (self.matrix[(first_x + 4) % self.rows][first_y] +
                              self.matrix[(second_x + 4) % self.rows][second_y])
...
if __name__ == "__main__":
    ...
    key = "SUPERSPY"
    cipher = PlayFair(encryption_key=key)
